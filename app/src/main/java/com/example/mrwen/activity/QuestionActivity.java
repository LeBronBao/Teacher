...
public class QuestionActivity extends AppCompatActivity {
    ...
    private void initView() {
        ...
        myIssue = (Issue) getIntent().getSerializableExtra("issue");

        mIid = myIssue.getId();
        StaticInfo.currentIssueId = mIid;
        tvTitle.setText(myIssue.getTitle());
        tvDetail.setText(myIssue.getContent());
        tvDiscoverNumber.setText(myIssue.getAnswerNumber() + "人回答");
        if (myIssue.isAnonymous()) {
            tvName.setText("匿名提问");
            ...
            final User user = myIssue.getUser();
            tvName.setText(user.getNickname());
            Glide.with(UiUtils.getContext()).load(UiUtils.getContext().getResources().getString(R.string.baseURL) + user.getImageURL())
    ...
    private void loadAnswers(final int iid) {
        ...
        call.enqueue(new Callback<ArrayList<Answer>>() {
            ...
            public void onFailure(Call<ArrayList<Answer>> call, Throwable t) {
                alertDialog.showAlertDialog(QuestionActivity.this,t.toString());
            }
        });
    }
    ...
    //获取单个问题
    private Issue getSingIssue(int iid){
        Retrofit retrofit=new Retrofit.Builder()
                .baseUrl(getResources().getString(R.string.baseURL))
                .addConverterFactory(GsonConverterFactory.create())
                .build();
        final InterfaceQuestion getIssue = retrofit.create(InterfaceQuestion.class);
        final Call<Issue> call = getIssue.getSingleIssue(iid);
        call.enqueue(new Callback<Issue>() {
            @Override
            public void onResponse(Call<Issue> call, Response<Issue> response) {
                if(response.body()!=null) {
                    myIssue = response.body();
                }
                else{
                    Toast.makeText(QuestionActivity.this, "获取问题失败", Toast.LENGTH_SHORT).show();
                }
            }
            @Override
            public void onFailure(Call<Issue> call, Throwable t) {
                Toast.makeText(QuestionActivity.this, t.toString(), Toast.LENGTH_SHORT).show();
            }
        });
        return myIssue;
    }
