...
public class UploadMultipleChoicesActivity extends Activity {
    ...
    private void retrofitImageUpload(String exerciseId){
        if(imageChange==1) {
            ...
            final Call<UniversalResult> call = imageUpload.multipleChoicesExerciseImageUpload(exerciseId, body);
            call.enqueue(new Callback<UniversalResult>() {
                @Override
                public void onResponse(Call<UniversalResult> call, Response<UniversalResult> response) {
                    if(response.body().getResultCode()==1){
                        ...
                        alertDialog.showAlertDialog(UploadMultipleChoicesActivity.this,"上传头像失败");
                    }
                }

                @Override
                public void onFailure(Call<UniversalResult> call, Throwable t) {
                    alertDialog.showAlertDialog(UploadMultipleChoicesActivity.this,t.toString());
                }
            });
        }
    }
